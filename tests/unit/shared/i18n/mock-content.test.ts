...
describe('shared/i18n/mock-content', () => {
  ...
  it('traduz termos conhecidos para en e es', () => {
    expect(translateMock('en', 'Navegação interior')).toBe('Inland navigation');
    expect(translateMock('es', 'Romaneio')).toBe('Lista de bultos');
  });

  it('retorna original quando não há tradução mapeada', () => {
    expect(translateMock('en', 'Termo não mapeado')).toBe('Termo não mapeado');
  });

  it('retorna string vazia para valores nulos/undefined', () => {
    expect(translateMock('en', null)).toBe('');
    expect(translateMock('en', undefined)).toBe('');
  });

  it('traduz listas item a item', () => {
    expect(translateMockList('en', ['Navegação interior', 'Romaneio'])).toEqual([
      'Inland navigation',
      'Packing list'
    ]);
